...
import {filterChildren, pickChildrenOfFirst} from '../../utils/ressources/children'
import {Breadcrumb, Button, Logo} from "epfl-elements-react-si-extra";
...
export const Base = ({ children } : Children) => {
  function classify (el : ReactElement) {
    for (const type of [Base.TopMenu, Base.AsideMenu, Base.Drawer,
  ...
  return <>
    <header role='banner' className='header' style={{display: "flex", justifyContent: "space-between"}}>
      <Logo />
    ...
    <div className='main-container'>
      <div style={{display: "flex", flexDirection: "row"}}>
        { breadcrumbs ? <Breadcrumb target='_top' homeUrl={'/roomcontrol'}>{breadcrumbs}</Breadcrumb> : <></> }
...
const defaultTopMenu: any[] = [] //{ link: 'https://www.epfl.ch/about/', anchor: 'About' },
